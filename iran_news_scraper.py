...
# تنظیم لاگ‌گیری برای دیدن خطاها در گیت‌هاب
logging.basicConfig(
    level=logging.INFO, 
...
class Scraper:
    ...
    def to_shamsi_string(self, date_str):
        """تبدیل رشته تاریخ به فرمت شمسی برای نمایش"""
        if not date_str:
            return "نامشخص"
        ...
        if len(numbers) >= 3:
            ...
            if 1300 <= y <= 1500:
                ...
                # این روش ساده است اما برای نمایش کلی کافیست
                shamsi_y = y + 621
                # برای ماه و روز تبدیل دقیق نیاز به کتابخانه دارد، اینجا تقریبی می‌گذاریم
                # یا می‌توانیم تاریخ میلادی را نگه داریم اما فرمت را شبیه شمسی کنیم
                # برای سادگی و جلوگیری از باگ، اگر میلادی بود، همان را با +621 نشان می‌دهیم
    ...
    def get_article_text(self, url):
        ...
        try:
            ...
            return paragraphs[:10], date_text, image, soup
        except Exception as e:
    ...
    def run(self):
        ...
        try:
            ...
            for item in links_to_process:
                ...
                try:
                    summary, date_text, image, _ = self.get_article_text(url)
